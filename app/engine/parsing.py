...
def _candidates(raw: str):
    ...
    repair_input = block if block is not None else raw
    repaired = repair_json(repair_input, return_objects=False)
    if isinstance(repaired, str) and repaired and repaired != repair_input:
        yield "json_repair", repaired
