...
update_gitignore() {
    ...
    if ! grep -qE "^\.agents/\.src/?$" "$gitignore" 2>/dev/null; then
        echo ".agents/.src/" >> "$gitignore"
    fi
...
main() {
    ...
    # ── Step 2: Update or Clone repository ────────────────────────────────

    local source_repo="$AGENTS_DIR/.src"
    mkdir -p "$AGENTS_DIR"

    local repo_updated=false
    if [[ -d "$source_repo/.git" ]]; then
        log_step "2/6" "Checking for updates (ref: $PLUGIN_VERSION)"
        local old_rev
        ...
        local new_rev
        new_rev=$(cd "$source_repo" && git rev-parse HEAD)
        if [[ "$old_rev" != "$new_rev" ]]; then
            log_success "Updated: $old_rev -> $new_rev"
            repo_updated=true
        else
            log_info "Already up to date at $old_rev"
        fi
    else
        log_step "2/6" "Cloning plugins (ref: $PLUGIN_VERSION)"
        git clone --depth 1 --branch "$PLUGIN_VERSION" "$REPO_URL" "$source_repo" &>/dev/null \
            || die "Failed to clone repository at ref '$PLUGIN_VERSION'"
        log_success "Repository cloned"
        repo_updated=true
    fi
    local SOURCE_DIR="$source_repo"
    ...
    if [[ -f "$SOURCE_DIR/AGENTS.md" ]]; then
        cp "$SOURCE_DIR/AGENTS.md" "$PROJECT_ROOT/AGENTS.md"
    ...
    local plugin_src="$SOURCE_DIR/plugins/10x-swe"
